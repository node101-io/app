...
const statusColors = {
  active: 'var(--active-status-color)',
  upcoming: 'var(--upcoming-status-color)',
  ended: 'var(--ended-status-color)'
};
const languageColors = {
  en: 'var(--active-status-color)',
  tr: 'var(--upcoming-status-color)',
  ru: 'var(--ended-status-color)'
};
const popularityColors = {
  high: 'var(--high-popularity-color)',
  medium: 'var(--medium-popularity-color)',
  low: 'var(--low-popularity-color)'
};
...
function getProjectLinkIconDOM(link) {
  if (link == 'web') {
    ...
    iconSVG.setAttributeNS(null, 'fill', 'var(--text-color)');
...
function uploadProjects() {
  ...
  const filters = {
    ...
    nin_id_list: stakeIds
  };

  if (!is_admin)
    filters.is_active = true;

  serverRequest('/stake/filter', 'POST', filters, res => {
    if (!res.success) {
    ...
    if (isUploadFistCall) {
      ...
      if (document.querySelector('.projects-wrapper')) {
        if (res.projects && res.projects.length) {
          ...
          document.querySelector('.projects-wrapper').innerHTML = 'No stakes found.';
        }
      }
    }
