  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.id == 'new-project-create-button') {
      ...
      const testnet = {
        ...
        testnet_url: document.getElementById('testnet-url-input').value,
        guide: [],
        requirements: [],
      };

      if (!testnet.name || !testnet.name.length)
      ...
      for (let i = 0; i < requirements.length; i++)
        testnet.requirements.push({
          name: requirements[i].childNodes[0].innerHTML,
          content: requirements[i].childNodes[2].innerHTML
        });

      serverRequest('/admin/testnet/create', 'POST', testnet, res => {
        if (res.success)
          return window.location = '/admin/testnet';
